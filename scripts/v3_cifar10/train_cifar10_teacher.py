"""Train the CIFAR-10 teacher model with hard labels."""
...
from kd_research.models import CIFAR10CNNTeacherModel


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train CIFAR-10 hard-label teacher.")
...
def main() -> None:
    ...
    model = CIFAR10CNNTeacherModel().to(device)
    loss_fn = nn.CrossEntropyLoss()
    optimizer = Adam(model.parameters(), lr=args.learning_rate)

    print("CIFAR-10 hard-label teacher")
    ...
    if args.checkpoint_path is not None:
        ...
        torch.save(
            {
                "model_name": "CIFAR10CNNTeacherModel",
                "dataset": "CIFAR-10",
                "device": str(device),
